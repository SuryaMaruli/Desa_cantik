...
$content = $content.Replace(
    "import plotly.express as px`r`nimport streamlit as st",
    "import plotly.express as px`r`nimport plotly.graph_objects as go`r`nimport streamlit as st"
)
$content = $content.Replace(
    "import plotly.express as px`nimport streamlit as st",
    "import plotly.express as px`nimport plotly.graph_objects as go`nimport streamlit as st"
)
...
def grafik_kategori(df, jenis):
    ...
    fig = px.bar(
        ...
        labels={"kategori": "", "nominal": "Nominal"},
        color="nominal",
        color_continuous_scale="Greens" if jenis == "Pemasukan" else "Reds",
...
def grafik_bulanan(df):
    ...
    fig.add_trace(
        go.Bar(
            ...
            opacity=0.82,
            hovertemplate="Bulan: %{x|%b %Y}<br>Pemasukan: Rp %{y:,.0f}<extra></extra>",
        )
    )
    fig.add_trace(
        go.Bar(
            ...
            opacity=0.82,
            hovertemplate="Bulan: %{x|%b %Y}<br>Pengeluaran: Rp %{y:,.0f}<extra></extra>",
        )
    )
    fig.add_trace(
        go.Scatter(
            ...
            name="Saldo",
            mode="lines+markers",
            line=dict(color=WARNA_JENIS["Saldo"], width=4, shape="spline"),
            marker=dict(size=9),
            hovertemplate="Bulan: %{x|%b %Y}<br>Saldo: Rp %{y:,.0f}<extra></extra>",
        )
    )
    fig.update_layout(
...
def grafik_pengeluaran_harian(df):
    ...
    harian = (
        pengeluaran.groupby(pengeluaran["tanggal"].dt.date)["nominal"]
        .sum()
        .reset_index()
    ...
    harian["Rata-rata 7 Hari"] = (
        harian["Pengeluaran"].rolling(window=7, min_periods=1).mean()
    )
    ...
    fig.add_trace(
        go.Scatter(
            ...
            mode="lines",
            line=dict(color="#7c3aed", width=3, dash="dot", shape="spline"),
...
def grafik_pengeluaran_bulanan(df):
    ...
    fig = px.bar(
        ...
        labels={"bulan": "", "Pengeluaran": "Pengeluaran"},
        color="Pengeluaran",
...
def grafik_treemap_kategori(df, jenis):
    ...
    fig.update_traces(
        ...
        hovertemplate="<b>%{label}</b><br>Nominal: Rp %{value:,.0f}<extra></extra>",
        marker=dict(line=dict(color="white", width=2)),
    )
...
$dashboardPattern = '(?ms)^            st\.write\(""\)\r?\n            section_header\(\r?\n                "Perkembangan Keuangan",.*?^            st\.write\(""\)\r?\n            section_header\(\r?\n                "Rekap Bulanan",'
$dashboardReplacement = $newDashboard + @'
...
$contentBaru = [regex]::Replace($content, $dashboardPattern, $dashboardReplacement, 1)
if ($contentBaru -eq $content) {
